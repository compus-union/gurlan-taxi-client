...
const { lat: destinationLat, lng: destinationLng } = storeToRefs(destinationStore)

onBeforeRouteLeave(async (to, from) => {
	if (to.meta.layout && to.meta.layout !== 'default') {
		const realLocationMarker = await mapsStore.findMarker('real-location-point')
		await mapsStore.removeMarker(realLocationMarker)
	}
})
...
const checkClient = async () => {
	...
	try {
		...
		if (!check) {
			...
			throw new Error('Qandaydir xatolik yuzaga keldi, dasturni boshqatdan ishga tushiring')
		}
...
onMounted(async () => {
	...
	try {
		...
		if (route.path === '/ride/setOrigin') {
			await mapsStore.loadMapDefault('map')
		}

		if (route.path === '/ride/setDestination') {
			await mapsStore.loadMapInDestinationPage('map')
		}

		if (route.path === '/ride/letsgo') {
			await mapsStore.loadMapLetsgoPage('map')
		}


		await mapsStore.addRealLocationPoint()
		...
		CapApp.addListener('appUrlOpen', async data => {
			...
			setTimeout(async () => {
				sharedMap.value?.setView(
					[destinationCoords?.latitude as number, destinationCoords?.longitude as number],
					defaultZoom.value
				)
...
watch(
	...
	async newOne => {
		...
		if (newOne.fullPath === '/ride/letsgo' || newOne.fullPath === '/ride/taxi') {
...
async function changeDestinationCoords(payload: { lat: number; lng: number }) {
	await destinationStore.changeCoords({ lat: payload.lat, lng: payload.lng }, 'void')
...
const showReverseGeocodingComponent = computed(() => {
	if (route.fullPath === '/ride/setOrigin' || route.fullPath === '/ride/setDestination') {
		return true
	}
...
<template>
	<div class="default-layout">
		<header v-if="displayErrorMessage === false" class="header fixed top-0 w-full h-auto z-50">
				...
				>
					...
					>
						<p class="font-manrope font-bold flex flex-row items-start w-[80%]">
							...
							<span class="right w-full text-left"
								><p class="text-[12px] font-poppins font-bold opacity-50">Qayerdan</p>
								<p class="text-lg overflow-hidden whitespace-nowrap text-ellipsis -mt-1">
					...
					>
						<p class="font-manrope font-bold flex flex-row items-start w-[80%]">
							...
							<span class="right w-full text-left"
								><p class="text-[12px] font-poppins font-bold opacity-50">Qayerga</p>
								<p class="text-lg overflow-hidden whitespace-nowrap text-ellipsis -mt-1">
		...
		<div id="map" class="map h-screen w-full z-[49]">
			<div v-if="displayErrorMessage" class="error-message mt-10 text-center">
				<h1 class="title text-foreground text-2xl font-bold">Xatolik yuzaga keldi</h1>
