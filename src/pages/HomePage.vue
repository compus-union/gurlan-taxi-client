...
import { CircleSlash2, Locate, MapPin, Search, Loader, ArrowLeft } from 'lucide-vue-next'
import { storeToRefs } from 'pinia'
import { loadingController } from '@ionic/vue'
import { Sheet, SheetClose, SheetContent, SheetTrigger, SheetHeader } from '@/components/ui/sheet'
...
const MainButton = defineAsyncComponent(() => import('@/components/ui/button/Button.vue'))
...
const { sharedMap, defaultZoom, markers, mapMoving, isSearching } = storeToRefs(mapsStore)
...
const debounce = ref<(fnc?: () => Promise<void>, delayMs?: number) => Promise<void>>(
	createDebounce()
)
...
onBeforeRouteLeave(async (to, from, next) => {
	if (to.path === '/ride/setDestination') {
		if (destinationLat.value && destinationLng.value) {
			sharedMap.value?.setView([destinationLat.value, destinationLng.value], defaultZoom.value)
		}
	}
	return next()
})
			...
			<div class="buttons flex flex-col space-y-2">
				...
				<Sheet>
					<SheetTrigger as-child>
						<MainButton class="w-full py-6 text-lg font-manrope font-semibold" variant="outline"
							><Search class="w-5 h-5 mr-2" /> Qidirish</MainButton
						>
					</SheetTrigger>
					<SheetContent class="h-screen overflow-hidden flex flex-col" side="bottom">
						<SheetHeader class="w-full flex items-start space-x-4 flex-row space-y-0">
							...
							<div class="titles text-left">
								...
								<SheetDescription
									>O'zingizga kerakli joy nomini izlang. Dehqon bozor, buyum bozor,
									6-maktab</SheetDescription
								>
							</div>
						</SheetHeader>
						<div
						...
						>
							<div class="form-part flex items-center justify-between">
								<Input
									...
									v-model="placeName"
									@input="
										debounce(async () => await searchPlacesStore.searchPlaces(placeName), 1000)
									"
							...
							>
								...
								>
									<SheetClose as-child>
										<button
											@click="changeOriginCoords({ lat: +place.lat, lng: +place.lon })"
